...
#Shouldn't the last one be False instead of True. It does not have an equal amount of English vowels and consonants.
...
# I could not finish the code in time but I wanted to compare if the first dictionary had the same ingredients as the second dictionary. Then I wanted to compare if the amounts were the same for the first and second dictionary.
...
def standings(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            lines = line.split()
            num = int(lines[1])
            numbers = p.append([num])
            #print(numbers)
            team = lines[0]

    # I made a list and tried to sort the numbers. I did not work. But I know sort should work on this
    
    return team
            
# --------------- DO NOT WRITE CODE BELOW THIS LINE -------------------------
if __name__ == '__main__':
    print(standings('Downloads/PAT resit 2025 F/PAT resit 2025 F/test1.txt')) #Expected output: 'Barcelona'
    print(standings('Downloads/PAT resit 2025 F/PAT resit 2025 F/test2.txt')) #Expected output: 'Napoli'
