...
## possibility to add .lower(), don't think we need that, but added as an extra.
...
#This is checking only the first one, we did miss something the pasta from the pasta from the first one, but if we do for loop we check only the first item, we want multiple items checked at once. if the, thats the only issue we are now facing values of d are in 
