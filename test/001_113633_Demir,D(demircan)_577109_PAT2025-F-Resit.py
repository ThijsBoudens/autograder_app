...
def balanced(s):
    ...
    for ch in s:
        if ch.isalpha() and ch in vowels:
            count_vowel +=1
        elif ch.isalpha() and ch in consonants:
            count_consonant +=1
...
def firstPrime(number):
    ...
    for i in range(1,10000000):
        if i > number and isPrime(i) and i not in lst:
            return i
