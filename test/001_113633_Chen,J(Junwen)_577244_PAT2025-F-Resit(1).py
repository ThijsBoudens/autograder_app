...
def balanced(s):
    # your code here
    #make everything in lower english letter and remove the extra part
    s = s.strip()
    ...
    #count vowels and letter
    for l in s:
        if l in letter:
            new +=l
        if l in vowels:
            res +=l
    #compare the length and return the results
    if len(new) == len(res):
        return True
    return False
...
def alphabetical(lst):
    # your code here
    #TURN everything into lowercase first,now x is the new list
    x = []
    for y in lst:
        y = y.lower()
        x.append(y)
...
def dinner(d1,d2):
    # your code here
    #check if they have the same elements,if something in d1 but not in d2, means False
    for x in d1.keys():
        if x not in d2.keys():
            return False
    #check the values
    if d1[x] > d2[x]:
        return False
    #the rest situation are all TRUE
    return True
